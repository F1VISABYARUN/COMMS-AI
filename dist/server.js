...
const followupProcessor_1 = require("./followupProcessor");
...
const env = nunjucks_1.default.configure('templates', {
    autoescape: true,
    express: app,
    watch: false
});
// Replicate Flask's url_for helper function
env.addGlobal('url_for', (type, options) => {
...
const SAMPLE_TRANSCRIPTS = {
    "insurance": {
        ...
        "text": ("Bob: Hey Sarah, I need to add my new delivery truck to my commercial policy. " +
    ...
    "real_estate": {
        ...
        "caller": "Rahul Reddy",
        "text": ("Rahul: Hello, I saw your listing for the 3BHK flat in Kondapur. " +
    ...
    "clinic": {
        ...
        "industry": "clinic",
        "caller": "Meera Sharma",
        "text": ("Meera: Hi, I'm calling to check if my thyroid lab results are ready. " +
    ...
    "coaching": {
        ...
        "text": ("Sandeep: Hello, I want to inquire about the GRE classroom coaching. " +
    ...
    "logistics": {
        ...
        "industry": "logistics",
        "caller": "Marcus Vance",
        "text": ("Marcus: Yes, this is Marcus from logistics. Shipment #45218 is stuck " +
...
app.get('/api/dashboard-stats', async (req, res) => {
    try {
        const { data: recordings, error } = await supabaseClient_1.supabase
            .from('recordings')
            .select('*')
...
app.post('/api/process', async (req, res) => {
    ...
    if (data.audio) {
        ...
        try {
            ...
            const aiResult = await (0, geminiProcessor_1.processCallAudioWithGemini)(audioBuffer, "Manual Upload");
            if (aiResult) {
                processedRecord = {
                    id: callId,
    ...
    try {
        const { error } = await supabaseClient_1.supabase.from('recordings').insert([processedRecord]);
        if (error) {
    ...
    if (processedRecord && processedRecord.result) {
        try {
            ...
            const actionItems = resObj.tasks ? resObj.tasks.map((t) => t.title).join(', ') : "";
...
app.get('/api/sync-status', async (req, res) => {
    ...
    try {
        ...
        const { data: pendingData, error: pendingErr } = await supabaseClient_1.supabase
            .from('recordings')
            .select('id')
...
app.post('/api/sync-trigger', async (req, res) => {
    try {
        ...
        await (0, followupProcessor_1.checkAndSendFollowups)();
...
app.post('/twilio/outbound-connect', (req, res) => {
    const dialTo = req.query.dial_to || '';
    const response = new twilio_1.default.twiml.VoiceResponse();
    ...
    (async () => {
        try {
            ...
            if (recordingUrl) {
                ...
                const audioBuffer = await (0, geminiProcessor_1.downloadTwilioRecording)(recordingUrl);
                if (audioBuffer && audioBuffer.length > 0) {
                    ...
                    aiResult = await (0, geminiProcessor_1.processCallAudioWithGemini)(audioBuffer, caller);
            ...
            if (!aiResult) {
                ...
                aiResult = await (0, geminiProcessor_1.processCallTranscriptWithGemini)(transcript, caller);
                record.result = aiResult;
            }
            if (aiResult) {
                ...
                while (retries > 0) {
                    try {
                        ...
                        retries--;
                        if (retries === 0)
                            console.error("[ERR] FATAL: Failed to write to Sheets after 3 attempts.", err);
                        else
            ...
            try {
                const { error } = await supabaseClient_1.supabase.from('recordings').insert([record]);
                if (error) {
...
app.post('/api/recordings/:id', async (req, res) => {
    ...
    try {
        ...
        if (result && data.source === 'manual') {
            ...
            const actionItems = result.tasks ? result.tasks.map((t) => t.title).join(', ') : "";
...
const server = app.listen(port, () => {
    ...
    (0, followupProcessor_1.startScheduler)();
});
